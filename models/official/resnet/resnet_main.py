...
def resnet_model_fn(features, labels, mode, params):
  ...
  if params['transpose_input'] and mode != tf.estimator.ModeKeys.PREDICT:
    ...
    features = tf.reshape(features, [4, 4, 1, -1])
    features = tf.transpose(features, [3, 0, 1, 2])  # HWCN to NHWC
    labels = tf.reshape(labels, [2, -1])
    labels = tf.transpose(labels, [1, 0])  # HWCN to NHWC
  ...
  if mode == tf.estimator.ModeKeys.EVAL:
    def metric_fn(labels, logits):
      ...
      """
      labelsmax = tf.argmax(labels, axis=1)
      predictions = tf.argmax(logits, axis=1)
      top_1_accuracy = tf.metrics.accuracy(labelsmax, predictions)
...
def _select_tables_from_flags():
  ...
  return [  # pylint: disable=g-complex-comprehension
      tfrecord_input.BigtableSelection(
          project=project,
          instance=instance,
          table=table,
          prefix=p,
...
def main(unused_argv):
  ...
  if FLAGS.tpu or params.use_tpu: #Insert 20210214 BorisTown Turtle X 
  ...
  if FLAGS.bigtable_instance:
    ...
    tfrecord_train, tfrecord_eval = [
        tfrecord_input.ImageNetBigtableInput(  # pylint: disable=g-complex-comprehension
    ...
    tfrecord_train, tfrecord_eval = [
        tfrecord_input.ImageNetInput(  # pylint: disable=g-complex-comprehension
            is_training=is_training,
  ...
  if FLAGS.mode == 'eval':
    ...
    for ckpt in tf.train.checkpoints_iterator(
      ...
      try:
        ...
        eval_results = resnet_classifier.evaluate(
            input_fn=tfrecord_eval.input_fn,
            steps=eval_steps,
    ...
    if FLAGS.mode == 'train':
      ...
      resnet_classifier.train(
          input_fn=tfrecord_train.input_fn,
      ...
      while current_step < params.train_steps:
        ...
        next_checkpoint = min(current_step + FLAGS.steps_per_eval,
                              params.train_steps)
        resnet_classifier.train(
            input_fn=tfrecord_train.input_fn, max_steps=next_checkpoint)
    ...
    if FLAGS.export_dir is not None:
      ...
      export_path = resnet_classifier.export_saved_model(
          ...
          serving_input_receiver_fn=tfrecord_input.image_serving_input_fn)
