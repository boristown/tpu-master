...
class ImageNetTFExampleInput(object):
  ...
  def replacenan(self,t):
      return tf.clip_by_value(t,1e-10,1.0)
  ...
  def dataset_parser(self, value):
    ...
    fix_price_len = 16
    keys_to_features = {
        'features': tf.FixedLenFeature([fix_price_len], tf.float32, default_value=[0.0]*fix_price_len),
        'label': tf.FixedLenFeature([], tf.float32, -1),
    }
    ...
    features = tf.reshape(features, [4,4,1])
    onehot_label = tf.one_hot(label, 2)
    features = self.replacenan(features)
    onehot_label = self.replacenan(onehot_label)
